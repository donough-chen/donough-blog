					...
					<div key={button.id} className='flex items-center gap-2'>
						...
						{button.type === 'wechat' || button.type === 'qq' ? (
							<div className='flex flex-1 items-center gap-2'>
								<input
									...
									accept='image/*'
									className='hidden'
									onChange={(e: React.ChangeEvent<HTMLInputElement>) => handleImageSelect(button.id, e)}
								/>
								{socialButtonImageUploads[button.id]?.type === 'file' ? (
									<div className='relative flex flex-1 items-center gap-2'>
										...
										<input
											type='text'
											value={button.value}
											onChange={(e: React.ChangeEvent<HTMLInputElement>) => handleUpdateButton(button.id, { value: e.target.value })}
						...
						{button.type !== 'email' && button.type !== 'wechat' && button.type !== 'qq' && (
							<input
								type='text'
								value={button.label || ''}
								onChange={(e: React.ChangeEvent<HTMLInputElement>) => handleUpdateButton(button.id, { label: e.target.value })}
								placeholder='标签文本（可选）'
