		...
		<motion.div initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay }} className='card relative'>
			...
			<div className='mt-3 space-y-2'>
				<textarea
					...
					value={form.summary}
					onChange={(e: React.ChangeEvent<HTMLTextAreaElement>) => updateForm({ summary: e.target.value })}
				/>
				...
				<div className='flex items-center gap-2'>
					<input
						...
						onChange={(e: React.ChangeEvent<HTMLInputElement>) => updateForm({ hidden: e.target.checked })}
